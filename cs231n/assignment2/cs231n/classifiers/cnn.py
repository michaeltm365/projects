...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        self.params["W2"] = np.random.randn(flat // 4, hidden_dim) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        dz2, grads["W3"], grads["b3"] = affine_backward(dz3, cache3)
        dz1, grads["W2"], grads["b2"] = affine_relu_backward(dz2, cache2)
        dz, grads["W1"], grads["b1"] = conv_relu_pool_backward(dz1, cache1)

        for i in range(1, 4):
          grads["W" + str(i)] += self.reg * self.params["W" + str(i)]

        weight_sum = np.sum(
          [np.sum(self.params["W" + str(i)] ** 2) for i in range(1, 4)])
