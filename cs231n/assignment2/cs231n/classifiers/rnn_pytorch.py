...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        word_to_vec = word_embedding_forward(captions_in, W_embed)

        if self.cell_type == "rnn":
          h = rnn_forward(word_to_vec, h0, Wx, Wh, b)
        
        scores = temporal_affine_forward(h.float(), W_vocab.float(), b_vocab.float())
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(N):
          ...
          while word != self._end:
            word_to_vec = word_embedding_forward(word, W_embed)

            next_h = rnn_step_forward(word_to_vec, h0, Wx, Wh, b)
            scores = temporal_affine_forward(next_h.float(), W_vocab.float(), b_vocab.float())
