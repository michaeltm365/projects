...
public class Model {
    ...
    public boolean atLeastOneMoveExists() {
        ...
        for (int i = 0; i < board.size() - 1; i++) {
            for (int j = 0; j < board.size() - 1; j++) {
                int currTileVal = tile(i, j).value();
                int topTileVal = tile(i, j + 1).value();
                int rightTileVal = tile(i + 1, j).value();
                if (currTileVal == topTileVal || currTileVal == rightTileVal) {
                    return true;
                }
            }
        }
        // check top row
        for (int i = 0; i < board.size() - 1; i++) {
            int currTileVal = tile(i, board.size() - 1).value();
            int rightTileVal = tile(i + 1, board.size() - 1).value();
            if (currTileVal == rightTileVal) {
                return true;
            }
        }
        // check rightmost column
        for (int j = 0; j < board.size() - 1; j++) {
            int currTileVal = tile(board.size() - 1, j).value();
            int topTileVal = tile(board.size() - 1, j + 1).value();
            if (currTileVal == topTileVal) {
                return true;
            }
        }
        return false;
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        boolean maxReached = targetY == board.size() - 1;
        while (!maxReached) {
            if (targetY == board.size() - 1) {
                maxReached = true;
            } else if (tile(x, targetY + 1) != null) {
                Tile tileAbove = tile(x, targetY + 1);
                if (!tileAbove.wasMerged() && currTile.value() == tileAbove.value()) {
                    targetY++;
